...
const plans = [
  { name: "Starter", price: { monthly: 15, yearly: 144 }, level: "starter" as const },
  { name: "Pro", price: { monthly: 49, yearly: 470 }, level: "pro" as const },
  { name: "Enterprise", price: { monthly: 99, yearly: 990 }, level: "all" as const },
];

export default function Pricing_Animated() {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4">
      ...
      <div className="flex justify-end mb-4 sm:mb-8">
        <div className="inline-flex items-center gap-2 text-xs sm:text-sm bg-zinc-100 dark:bg-zinc-900 p-1 rounded-lg">
          <button
            onClick={() => setIsYearly(false)}
            className={cn(
              "px-3 py-1 rounded-md transition-all",
              !isYearly ? "bg-white dark:bg-zinc-700 shadow" : "text-zinc-500"
            )}
          >
            Monthly
          </button>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-8">
        {plans.map((plan) => (
          <motion.button
            ...
            className={cn(
              "flex-1 p-5 rounded-xl text-left transition-all border",
              selectedPlan === plan.level
                ? "border-blue-500 shadow-lg bg-blue-50/30 dark:bg-blue-900/20"
      ...
      <div className="mt-8 text-center">
        <Button className="w-full sm:w-auto bg-blue-500 hover:bg-blue-600 text-white px-8 py-2 rounded-xl flex items-center mx-auto">
...
function shouldShowCheck(included: Feature["included"], level: string) {

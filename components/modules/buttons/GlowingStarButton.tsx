...
interface GlowingStarButtonProps {
  name?: string;
  text?: string;
}
...
export const  GlowingStarButton: React.FC<GlowingStarButtonProps> = ({
  ...
  return (
    <button className="group relative rounded-full p-px overflow-hidden dark:bg-neutral-800 bg-neutral-200">
      {/* OUTER GLOW LAYER */}
      ...
      {/* MAIN CONTENT */}
      <span className="relative z-[1] flex items-center justify-center gap-2 dark:bg-neutral-950/90 bg-neutral-50/90 rounded-full py-2 px-4 pl-3 w-full">
        {/* ICON */}
        ...
        {/* TEXT */}
        <span className="bg-gradient-to-b ml-1.5 dark:from-white dark:to-white/60 from-neutral-950 to-neutral-950/60 bg-clip-text text-xs text-transparent group-hover:scale-105 transition-transform">
          {text ?? `Star ${name} on Github`}
        </span>
      </span>
    </button>
  );
